...
def select_series_title_with_most_human_characters
  "select series.title as species_count from series
inner join characters on series.id = characters.series_id
group by characters.species order by species_count asc limit 1"
end

def select_character_names_and_number_of_books_they_are_in
  "select characters.name, count(character_books.book_id) as appearances from characters
